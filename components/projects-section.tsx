          ...
          {projects.map((p) => (
            <div key={p.slug}
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="flex flex-wrap gap-1.5">{p.tags.map(tag => <Tag key={tag} label={tag} />)}</div>
                <Link href={`/projects/${p.slug}`} className="inline-flex items-center gap-1.5 text-sm font-semibold text-violet-600 dark:text-violet-400 hover:text-violet-700 dark:hover:text-violet-300 transition-colors group/btn">
                  {t.projects.viewDetails}<ArrowRight className="w-3.5 h-3.5 group-hover/btn:translate-x-1 transition-transform" />
                </Link>
              </div>
            </div>
          ))}
